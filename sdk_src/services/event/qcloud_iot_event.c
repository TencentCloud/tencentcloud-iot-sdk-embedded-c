...
/**
* @brief check the return value of snprintf()
*
* @param returnCode		return value of snprintf()
* @param maxSizeOfWrite	Max size of write buffer
* @return 				QCLOUD_RET_SUCCESS when value OK, or error code when something wrong
*/
...
{	 
	...
	if(event_count > SIGLE_EVENT){// more than one event
...
int qcloud_iot_post_event(void *pClient, char *pJsonDoc, size_t sizeOfBuffer, uint8_t event_count, sEvent *pEventArry[], OnEventReplyCallback replyCb) 									                                           
{
    ...
    Qcloud_IoT_Client* mqtt = (Qcloud_IoT_Client*)pshadow->mqtt;
    
	rc = _IOT_Construct_Event_JSON(pClient, pJsonDoc, sizeOfBuffer, event_count, pEventArry, replyCb, mqtt->command_timeout_ms);
	if (rc != QCLOUD_RET_SUCCESS) {
...
int qcloud_iot_post_event_raw(void *pClient, char *pJsonDoc, size_t sizeOfBuffer, char *pEventMsg, OnEventReplyCallback replyCb)                                            
{
	...
	rc = _IOT_Event_JSON_Init(pshadow, pJsonDoc, sizeOfBuffer, MUTLTI_EVENTS, replyCb, mqtt->command_timeout_ms);
	if (rc != QCLOUD_RET_SUCCESS) {
		Log_e("event json init failed: %d", rc);
		return rc;
	}
