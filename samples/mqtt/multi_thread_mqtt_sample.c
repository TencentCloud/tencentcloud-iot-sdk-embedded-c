...
static unsigned int sg_rx_count_array[MAX_PUB_THREAD_COUNT][PUBLISH_COUNT]; // record the times when msg from subscribed topic is received
...
static int _setup_connect_init_params(MQTTInitParams* initParams)
{
    ...
    HAL_Snprintf(initParams->cert_file, FILE_PATH_MAX_LEN, "%s/%s/%s", current_path, certs_dir, sg_device_info.dev_cert_file_name);
    HAL_Snprintf(initParams->key_file, FILE_PATH_MAX_LEN, "%s/%s/%s", current_path, certs_dir, sg_device_info.dev_key_file_name);
#else
    initParams->device_secret = sg_device_info.device_secret;
#endif

    memset(sg_pub_sub_test_topic, 0, MAX_SIZE_OF_TOPIC_CONTENT);
    HAL_Snprintf(sg_pub_sub_test_topic, MAX_SIZE_OF_TOPIC_CONTENT, "%s/%s/data", sg_device_info.product_id, sg_device_info.device_name);
...
static void _mqtt_publish_thread_runner(void *ptr)
{
    ...
    for (itr = 0; itr < PUBLISH_COUNT; itr++) {
        int size = HAL_Snprintf(topic_content, sizeof(topic_content), "{\"action\": \"thread-%d\", \"count\": \"%d\"}", thread_id, itr);
...
static int _mqtt_multi_thread_test(void* client)
{
    ...
    HAL_Printf(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n");
    if (RX_RECEIVE_PERCENTAGE <= percent_of_rx_msg &&
        0 == sg_rx_msg_buf_too_big_count &&
        0 == sg_rx_unexpected_count ) {
        // test success
        HAL_Printf("Success! PercentOfRxMsg: %f %%\n", percent_of_rx_msg);
        HAL_Printf("Published Messages: %d , Received Messages: %d \n", PUBLISH_COUNT * MAX_PUB_THREAD_COUNT, rx_msg_count);
