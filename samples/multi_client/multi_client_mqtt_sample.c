...
#define OS_PATH "./"
    ...
    char *device_info_file[MAX_MQTT_THREAD_COUNT] = {
        OS_PATH"cert_device_info1.json", OS_PATH"cert_device_info2.json", OS_PATH"cert_device_info3.json"
    };
#else
    char *device_info_file[MAX_MQTT_THREAD_COUNT] = {
        OS_PATH"psk_device_info1.json", OS_PATH"psk_device_info2.json", OS_PATH"psk_device_info3.json"
    };
#endif
...
static int _publish_test_msg(void *client, char *topic_keyword, QoS qos, int count, int thread_id)
{
    ...
    int size = HAL_Snprintf(topic_name, sizeof(topic_name), "%s/%s/%s", dev_info->product_id, dev_info->device_name, topic_keyword);
    ...
    size = HAL_Snprintf(topic_content, sizeof(topic_content), "{\"text\": \"thread-%u\", \"count\": \"%d\"}", thread_id, count);
...
static void _on_message_callback(void *pClient, MQTTMessage *message, void *user_data)
{
    ...
    Log_i("Thread-%d recv msg topic:%.*s, payload:%.*s", app_data->thread_id,
...
{
    int c;
    while ((c = utils_getopt(argc, argv, "l:")) != EOF)
        switch (c) {
            case 'l':
